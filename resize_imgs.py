...
def scale_img(input_path, output_path, width=256, height=256, log=True):
    img = Image.open(input_path)
    
    (cur_width, cur_height) = img.size
    
    deform = min(width / cur_width, height / cur_height)
    if(log):
    ...
    im_new = Image.new("RGB", (width, height), color='white')
    im_new.paste(img.resize(desired_size, Image.BICUBIC), (delta_x // 2, delta_y // 2))
    im_new.save(output_path)

def main(image_dir, data_file, output_dir, width=256, height=256, num_tasks = 32, log=True):
    img_data = pandas.read_csv(data_file)
    ...
    for img in img_list:
        pool.apply_async(scale_img, (os.path.join(image_dir, img) + ".jpg", os.path.join(output_dir, img) + ".jpg", width, height, log))
...
if __name__ == "__main__":
    ...
    main(args.image_dir, args.data_file, args.output_dir, width=args.s, height=args.s)
